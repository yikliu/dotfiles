...
-------------------------------------------------------------------------------
-- Bemol Integration (monorepo workspace support)
-------------------------------------------------------------------------------
-- Reads .bemol/ws_root_folders to add workspace folders to LSP.
-- This is a no-op if no .bemol directory exists.
-- Resolve symlinks in a path to match what bemol generates
...
function M.setup_bemol()
	...
	local ws_folders = {}
	local file = io.open(resolve_path(bemol_dir) .. "/ws_root_folders", "r")
	if file then
		for line in file:lines() do
			table.insert(ws_folders, resolve_path(line))
		end
		file:close()
	end
...
function M.find_bemol_root()
	local bemol_dir = vim.fs.find({ ".bemol" }, { upward = true, type = "directory" })[1]
	if bemol_dir then
		return resolve_path(vim.fn.fnamemodify(bemol_dir, ":h"))
	end
	return nil
end
...
function M.on_attach(client, bufnr)
	...
	-- Diagnostic float: use <space>dl to show manually
	...
	-- Semantic tokens are handled automatically by the LSP client in Neovim 0.10+
