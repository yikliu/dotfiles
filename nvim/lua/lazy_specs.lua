...
local plugin_specs = {
    ...
    -----------------------------------------------------------------------
    -- Fast buffer jump
    { "rlane/pounce.nvim", event = "VeryLazy", config = function() require("config.nvim-pounce") end },
    -- Search match counter
    { "kevinhwang91/nvim-hlslens", branch = "main", keys = { "*", "#", "n", "N" }, config = function() require("config.hlslens") end },
    -- Modern matchit
    ...
    -- Git linker
    { "ruifm/gitlinker.nvim", event = "User InGitRepo", config = function() require("config.git-linker") end },
    -- Git log viewer
    ...
    -----------------------------------------------------------------------
    -- Yank history
    { "gbprod/yanky.nvim", cmd = { "YankyRingHistory" }, config = function() require("config.yanky") end },
    -- Undo tree
    ...
    -- URL opener
    { "sontungexpt/url-open", event = "VeryLazy", cmd = "URLOpenUnderCursor", config = function() local ok, url = pcall(require, "url-open"); if ok then url.setup({}) end end },
    -- Open in browser
    ...
    -----------------------------------------------------------------------
    -- Mac keyboard switch
    { "lyokha/vim-xkbswitch", enabled = function() return vim.g.is_mac and utils.executable("xkbswitch") end, event = { "InsertEnter" } },
    -- Windows IME
    ...
    -----------------------------------------------------------------------
    -- Edit browser text
    { "glacambre/firenvim", enabled = function() return vim.g.is_win or vim.g.is_mac end, build = function() vim.fn["firenvim#install"](0) end, lazy = true },
