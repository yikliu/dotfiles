...
keymap.set("n", "<leader>sv", function()
  ...
  -- Re-run lazy plugin configs
  local skip = { ["lazy.nvim"] = true }
  local plugins = require("lazy").plugins()
  for _, plugin in ipairs(plugins) do
    if plugin._.loaded and type(plugin.config) == "function" and not skip[plugin.name] then
      local ok, err = pcall(plugin.config, plugin)
      if not ok then
        vim.notify("Failed to reload " .. plugin.name .. ": " .. err, vim.log.levels.WARN)
      end
    end
  end
